...
- **The advanced-operator surface.** site:, filetype:, exact phrase, exclusions,
  before:/after: date bounds, freshness windows, news/image/video verticals. All free,
  all server-side, all reachable once the profile is warmed.
...
def google_search(
    ...
    """Search Google. Ads are stripped; results are {rank, title, url, host, snippet}.
    ...
    assemble the correct syntax for you:
      ...
      vertical='news'           web | news | images | videos | shopping
    ...
    depth only when you actually need it.
