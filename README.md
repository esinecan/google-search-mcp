...
- **The advanced-operator surface** — `site:`, `filetype:`, exact phrase, exclusions,
  `before:`/`after:` bounds, freshness windows, news/image/video verticals, region and language
  bias. All server-side, all free once the transport works. This is the strongest reason to
  use it; no keyword API has an equivalent.
...
| tool | what it is for |
|---|---|
| `google_search` | one query, ads stripped, full operator surface |
